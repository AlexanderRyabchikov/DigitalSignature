...
void A_func(unsigned char Y[], unsigned char R[])
{
    for (int i = 0; i < 24; i++) 
        R[i] = Y[i + 8];
    for (int i = 0; i < 8; i++) 
...
bool gost(char* message, size_t nSize)
{                                                               
	unsigned char result[32], block[32], Sum[32], L[32], H[32], newH[32];
	unsigned int pos = 0, posIB = 0;                                                                                                                 
    memset(Sum, 0, sizeof (Sum));
    memset(H, 0, sizeof (H));
    while ((posIB < nSize) || pos) 
    {
        if (posIB < nSize) 
	...
	for (int i = 0; i < 32; i++)
	{
		snprintf(&message[i], sizeof(result), "%x", result[i]);
	}
